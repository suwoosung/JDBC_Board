...
public class JDBCProcess {
	 ...
	 *    페이스를 구현한 클래스.(기본 자바api에 없기때문에 별도의 다운로드 필요)
	 *  2)DriverManger : Driver들을 관리하는 클래스. 여러개의 Driver들중
	 *  어떤 Driver를 통해 db와 연결할지 선택할 수 있다.
	 *  3)Connection : DB와 연결된 상태임을 나타내는 객체. 연결 설정 및
	 *   해제와 관련된 메서드 , Statement생성 및 트랜잭션 관리가 가능하다.
	 *  4)(Prepared)Statement : 연결된 DB에 SQL문을 전달하고 실행한 후
	 *     결과를 받아내는 객체.
	 *  5) ResultSet : 실행한 sql문이 DQL문일 경우 반환되는 결과를 저장하는
	 *  객체
	 * */
